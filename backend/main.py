...
from ai_engine import analyze_market, analyze_competitors, analyze_risks, generate_scorecard, analyze_heatmap

app = FastAPI(title='StartupIQ AI Backend', version='1.0.0')
...
@app.get('/')
def home():
    return {'message': 'StartupIQ Backend Running', 'status': 'healthy'}
...
def full_analysis(request: IdeaRequest):
    ...
    scorecard = generate_scorecard(idea, market.get('market_demand_score', 70), competitors.get('saturation_level', 'medium'), market.get('trend_direction', 'growing'))
    heatmap = analyze_heatmap(idea)
    return {'idea': idea, 'market': market, 'competitors': competitors, 'risks': risks, 'scorecard': scorecard, 'heatmap': heatmap}